...
export function useDefaultsFromURLSearch():
  ...
  useEffect(() => {
    ...
    const parsed = queryParametersToSwapState(query, USDC[chainId]?.address, GOLD[chainId]?.address)
  ...
  useEffect(() => {
    ...
    const fetchAndUpdatePairPrice = async () => {
      ...
      if (data) {
        ...
        let hasEnoughLiquidity = pairHasEnoughLiquidity(data, timeWindow)
        if (chainId === ChainId.BSC_TESTNET) hasEnoughLiquidity = true
